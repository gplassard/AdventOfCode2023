...
object Day11 {
  ...
  def solve(lines: List[String], multiplier: BigInt): BigInt = {
    ...
    val expandedRowsIndices = lines.zipWithIndex
      .filter(l => l._1.forall(_ == '.'))
      .map(_._2)
      .map(BigInt.apply)
      .toSet
    val expandedColsIndices = rotate(galaxy).zipWithIndex
      .filter(l => l._1.forall(_ == '.'))
      .map(_._2)
      .map(BigInt.apply)
      .toSet
  ...
  private def cartesianProduct(
      as: Set[(BigInt, BigInt)],
      bs: Set[(BigInt, BigInt)]
  ): Set[((BigInt, BigInt), (BigInt, BigInt))] = for {
    a <- as; b <- bs
  } yield (a, b)
  ...
  private def computeDistance(
      a: (BigInt, BigInt),
      b: (BigInt, BigInt),
      expandedRowsIndices: Set[BigInt],
      expandedColsIndices: Set[BigInt],
      multiplier: BigInt
  ): BigInt =
    val deltaR = abs(b._1 - a._1)
    val deltaC = abs(b._2 - a._2)
    val doubleR = expandedRowsIndices.filter(r =>
      min(a._1, b._1) < r && r < max(a._1, b._1)
    )
    val doubleC = expandedColsIndices.filter(c =>
      min(a._2, b._2) < c && c < max(a._2, b._2)
    )
