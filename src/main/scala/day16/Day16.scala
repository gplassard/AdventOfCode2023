...
object Day16 {
  enum Direction(val deltaRow: Int, val deltaColumn: Int, val isHorizontal: Boolean) {
  ...
  def next(beam: Beam, grid: List[List[Char]]): (Option[Beam], Option[Beam]) = {
    val nextPosition = (beam.x + beam.direction.deltaRow, beam.y + beam.direction.deltaColumn)
