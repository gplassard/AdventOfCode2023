...
object Day08 {
  ...
  private def count(start: String, instructions: List[Char], paths: Map[String, (String, String)], stop: String => Boolean): Int =
  ...
  private def nextPosition(paths: Map[String, (String, String)], current: String, instruction: Char): String =
    if (instruction == 'L') paths(current)._1 else paths(current)._2

  private def makePaths(lines: List[String]): Map[String, (String, String)] = lines.drop(2).map { line =>
  ...
  def part2(lines: List[String]): BigInt =
    val instructions = lines(0).toList
    val paths = makePaths(lines)
    val counts = paths.keys.filter(_.endsWith("A"))
      .map(position => count(position, instructions, paths, s =>  s.endsWith("Z")))
      .map(BigInt.apply)
      .toSeq
    lcm(counts)
}
