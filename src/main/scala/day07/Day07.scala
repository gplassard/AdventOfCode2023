...
object Day07 {
  ...
  private def solve[T](parsedLines: List[(T, Int)])(implicit ordering: Ordering[T]): Int = {
    parsedLines
      .sortBy((hand, _) => hand)
      .reverse
      .zipWithIndex
  ...
  implicit object HandPart2Ordering extends Ordering[HandPart2] {
    private val cardRank: Seq[Char] = List('A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J')
  ...
  case class HandPart2(cards: List[Char]) extends Hand {
    def handType(): HandType = {
      ...
      grouped.filter(_._1 != 'J').view.mapValues(_.size) match {
        case m if (m.values.maxOption.getOrElse(0) + jokerCount) == 5 => HandType.FiveOfAKind
        case m if (m.values.max + jokerCount) == 4 => HandType.FourOfAKind
        case m if differentCards == 2 && (m.values.max + jokerCount) == 3 => HandType.FullHouse
  ...
  def part2(lines: List[String]): Int = solve(lines
    .map { line =>
