...
object Day23 {
  ...
  def longestPath(
    ...
    while (queue.nonEmpty) {
      ...
      if (position == to) {
        ...
        val neighbors =
          findNeighbors(position, grid, isV1).filterNot(visited.contains)
        neighbors.foreach(n =>
          queue.enqueue((n, distance + 1, visited + position))
        )
      }
    }
    maxDistance
  ...
  def contract(
      ...
      isV1: Boolean
  ): MMap[(Int, Int), MMap[(Int, Int), Int]] = {
    val pois = findPois(grid, isV1) + start + end
    val distances =
      MMap.from(pois.map(poi => poi -> MMap.empty[(Int, Int), Int]))
    for {
      poi <- pois
    } {
      ...
      while (todos.nonEmpty) {
        ...
        if (distance != 0 && pois.contains(todo)) {
          ...
          val neighbors =
            findNeighbors(todo, grid, isV1).filterNot(seen.contains)
          for {
            neighbor <- neighbors
          } {
  ...
  def dfs(
      from: (Int, Int),
      to: (Int, Int),
      distances: MMap[(Int, Int), MMap[(Int, Int), Int]],
      visited: Set[(Int, Int)]
  ): Int = {
    if (from == to) {
      0
      ...
      nexts
        .map(n => dfs(n._1, to, distances, visited + from) + n._2)
        .maxOption
